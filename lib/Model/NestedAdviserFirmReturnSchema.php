...
/**
 * NestedAdviserFirmReturnSchema Class Doc Comment
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 ...
 */
class NestedAdviserFirmReturnSchema implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'NestedAdviserFirmReturnSchema';
    ...
    */
    ...
    public function setPhoneNumber($phone_number)
    {
        ...
        if ((!preg_match("/^\\s*(?:\\+?(\\d{1,6}))?[-.\\s(]*(\\d{1,6})[-.\\s)]*(\\d{1,6})([-.\\s]*(\\d{1,6})[-.\\s]*)?([-.\\s]*(\\d{1,6})[-.\\s]*)?([-.\\s]*(\\d{1,6})[-.\\s]*)?(?: *(\\d+))?\\s*$/", ObjectSerializer::toString($phone_number)))) {
            throw new \InvalidArgumentException("invalid value for \$phone_number when calling NestedAdviserFirmReturnSchema., must conform to the pattern /^\\s*(?:\\+?(\\d{1,6}))?[-.\\s(]*(\\d{1,6})[-.\\s)]*(\\d{1,6})([-.\\s]*(\\d{1,6})[-.\\s]*)?([-.\\s]*(\\d{1,6})[-.\\s]*)?([-.\\s]*(\\d{1,6})[-.\\s]*)?(?: *(\\d+))?\\s*$/.");
        }
